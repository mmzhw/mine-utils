...
function convertToH265(filePath) {
    ...
    try {
        ...
        const ffmpegCommand = `ffmpeg -y -i "${filePath}" -c:v hevc_nvenc -b:v ${Math.floor(originalBitrate / 1000)}k -maxrate ${Math.floor(originalBitrate / 1000)}k -bufsize ${Math.floor(originalBitrate / 500)}k -preset p5 -c:a copy -map 0 "${tempFilePath}"`;
